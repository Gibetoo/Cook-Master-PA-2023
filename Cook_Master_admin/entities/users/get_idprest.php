...
function getidPrest(string $id_metier): string
{

    $url = 'http://localhost/Projet-Annuel/Database/index?demande=prestataire&id_metier='. $id_metier; // On définit l'URL du serveur
    ...
    if ($response["success"] == true) { // Si la création de l'utilisateur a réussi, on affecte la réponse à $results
        return $response["message"]['nom_metier'];
